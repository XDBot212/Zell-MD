    ...
    return new Promise(async(resolve, reject) => {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (xp > 200)
                    this.sendButton(m.chat, `[❗] *Sepertinya Anda Bermain Curang, Menggunakan Calculator*`, author, null, [['𝗕𝗨𝗬 𝗟𝗜𝗠𝗜𝗧', '.buy limit'], ['「 𝗠 𝗘 𝗡 𝗨 」', '.zellmenu']] , m)
                else
                    m.exp += xp
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.sendButton(m.chat, `[❗] *Limit anda habis, Beberapa command tidak bisa diakses*`, author, null, [['「 𝗕𝗨𝗬 𝗟𝗜𝗠𝗜𝗧 」', '.buy limit'], ['「 𝗠 𝗘 𝗡 𝗨 」', '.zellmenu']] , m)
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    m.reply(`*[❗] Halo Owner*\n\n_Laporan Error terdeteksi_\n\nError terjadi pada 🗂️ Plugin:* ${m.plugin}\n*📤 Dari:* ${m.sender}\n*🗳️ID:* ${m.chat}\n*📑 Command Error:* ${usedPrefix}${command} ${args.join(' ')}\n⚠️ *Logs Eror:*\n\n\`\`\`${text}\`\`\``.trim(), data.jid)
                            }
                        m.reply(text)
                    }
                } finally {
...
export async function participantsUpdate({ id, participants, action }) { 
 ...
             if (chat.welcome) {
    ...
    this.sendHydrated(id, text, wm + '\n\n' + botdate, pp, sgc, (action == 'add' ? '💌 Welcome' : '👋 Goodbye'), user.split`@`[0], '👤 User', [
      ['「 𝗠 𝗘 𝗡 𝗨 」', '.zellmenu'],
      [(action == 'add' ? '\n\nWelcome!:)' : '\n\nGoodbye!:('), '...'],
      [null, null]
...
*.delete*`, author, [['Off Feature', '.off antidelete'],['Menu', '.menu']], msg, adReply)
...
global.dfail = (type, m, conn) => {
    ...
    let nmsr = `👋 Halo *@${m.sender.split("@")[0]}*`
    let msg = {
        rowner: `${nmsr}\n 
...
Fitur ini di *disable* !`
    ...
    if (msg) return conn.sendButton(m.chat, danied, msg, `${imgr + 'Accses Danied'}`, [['「 𝗠 𝗘 𝗡 𝗨 」', '.zellmenu'],['「 𝗢 𝗪 𝗡 𝗘 𝗥 」', '.zellowner']],m)
    
     let msgg = {
    	unreg: `${nmsr}\nSilahkan registerasi ke database terlebih dahulu untuk menggunakan bot ini lebih lanjut *Click button di bawah*\n\n*Kalian bisa ikuti langkah Register selanjutnya*`
}[type]
if (msgg) return conn.sendButton(m.chat, `${global.htki} Register ${global.htka}`, msgg, `${imgr + 'Verify'}`, [['Register', '.zellreg']],m)
}
